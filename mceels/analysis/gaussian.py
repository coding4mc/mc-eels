...
def get_cropped_x_axis(
    ...
    signal_range_min_index = np.argmin(np.abs(x_axis - signal_range_min))
    signal_range_max_index = np.argmin(np.abs(x_axis - signal_range_max))

    x_signal_range = x_axis[signal_range_min_index : signal_range_max_index]
    y_signal_range = y_axis[signal_range_min_index : signal_range_max_index]
...
def fit_double_gaussian_single_spectum(
        y_axis: np.ndarray,
        x_axis: np.ndarray,
        signal_range: Tuple[int, int],
        peak_range: float
    ...
    initial_center_0 = x_peak - 0.1
    initial_center_1 = x_peak + 0.1
    initial_sigma = 1.0
...
def fit_double_gaussian(
    ...
    for index, data_record in enumerate(signal.data[0]):
        coeff = fit_double_gaussian_single_spectum(
            y_axis=data_record,
            x_axis=all_x,
