...
def get_x_array(signal: EELSSpectrum):
    """
...
def get_y_array(signal: EELSSpectrum):
    """
    Returns an array of the Y-axis of the image.
    
    Parameters:
    ----------
    signal : HyperSpy signal
        Signal to get Y-axis from
    ...
    numpy.ndarray
        Y-axis array
    """
    y_scale = signal.axes_manager.navigation_axes[1].scale
    y_size = signal.axes_manager.navigation_axes[1].size
    return np.arange(y_size) * y_scale
